...
# Path: transform.py
SCORM_FILES = "https://drive.google.com/drive/folders/14UM4yOWZfyL65cluoEQd8W7dcalDbmRf?usp=sharing"  # noqa: E501

SCORM_FILES_DRIVE_ID = "14UM4yOWZfyL65cluoEQd8W7dcalDbmRf"

# out of school resources:
INTRO_ID = "https://360.articulate.com/review/content/7dc742aa-98e1-49c0-a0cf-1ac4d54e30a0/review"  # noqa: E501
...
.nav-control__menu, .nav-sidebar__body, .lesson-nav-link,
...
.lesson-nav__link-icon, .nav-control__wrapper,
.navButtonsFloating__prev.navButtonsFloating__prev--entered,
...
function hideDiv() {
  document.querySelectorAll('div.fr-view').forEach(function(el) {
    if (el.textContent.trim() === "LET'S START LEARNING!") {
      var parentDiv = el.closest('div.block-text--onecol');
      if (parentDiv) {
    ...
    document.addEventListener('animationend', function(event) {
      if (event.animationName === 'fadeIn') {
        ...
        for (var i = 0; i < allInnerElements.length; i++) {
          ...
          if (element.textContent.trim() === "LET'S START LEARNING!"
              || element.textContent.trim() === "LET'S CONTINUE LEARNING!")  {
            element.closest('div.block-text--onecol').style.display = 'none';
            break;
          }
        }
      }
